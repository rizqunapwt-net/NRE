...
const SalesQuotesPage: React.FC = () => {
    ...
    const { data = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            try { const res = await api.get('/finance/sales-quotes'); return res.data; }
            catch { return []; }
        },
    });
    ...
    return (
        <div>
            <Breadcrumb className="mb-4" items={[{ title: 'Beranda' }, { title: 'Penjualan' }, { title: 'Penawaran Penjualan' }]} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <Title level={4} style={{ margin: 0 }}>Penawaran Penjualan</Title>
                <Space>
